...
import { Attendee } from "../party/types/attendee"

describe("FilesHandles", () => {
  let fileHandler: FilesHandlerService
  ...
  beforeEach(async () => {
    ...
    fileHandler = app.get<FilesHandlerService>(FilesHandlerService)
  })

  describe(" (File Handler)", () => {
    it("Parse raw text file into a typed list of all its valid elements", () => {
      const input = `{"latitude": "52.986375", "user_id": 12, "name": "Christina McArdle", "longitude": "-6.043701"}
        ...
        `
      const attendees: Attendee[] = fileHandler.parseFileBuffer<Attendee>(
        Buffer.from(input),
        "\n",
        isOfAttendee
      )
      expect(attendees.length).toBe(32)
    })

    it("Parses out invalid json tokens", async () => {
      const input = `sa9
        2m 239
        32 ][ 32
            {"latitude": "52.833502", "user_id": 25, "name": "David Behan", "longitude": "-8.522366"}
            03 rm]`
      expect(
        fileHandler.parseFileBuffer<Attendee>(
          Buffer.from(input),
          "\n",
          isOfAttendee
        ).length
    ...
    it(`Parses input with a mix of invalid tokens, valid json objects too but not attendee types`, () => {
      // json in the mix, missing user_id
      const input = `sa9
      2m 239
      32 ][ 32
          {"latitude": "52.833502", "name": "David Behan", "longitude": "-8.522366"}
          03 rm]`
      expect(
        fileHandler.parseFileBuffer<Attendee>(
          Buffer.from(input),
...
function isOfAttendee(obj: any): obj is Attendee {
  return (
